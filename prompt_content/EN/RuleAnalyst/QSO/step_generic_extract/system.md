...
## Task
Extract the analysis conclusion section from the given QSO spectral analysis reasoning text, and output structured JSON.
...
- **Hypothesis**: Complete description of the line-matching hypothesis, **strict format**: `line_name-obs_wavelength, line_name-obs_wavelength, ..., @ z≈redshift`.
  ...
  - Example: `3854.006-Mg II, 5128.678-O[II], 6821.675-O[III]4959, 6887.702-O[III]5007, 9040.6-Hα @ z≈0.376`
...
- **Physical_type**: Astrophysical type, one of the following values:
  - `Case 1 (Typical QSO)`: Continuous spectrum with higher blue end, broad emission lines (Lyα, C IV, C III]、Mg II etc.)
  - `Case 2 (Host galaxy-dominated AGN)`: Continuous spectrum dominated by host galaxy, narrow emission lines (O[III]、O[II]、Hβ、Hα etc.)
  - If undeterminable, fill `null`
...
- **Adopted_pairs**: Finally adopted line pairs, in list form. Each element is the adoption result for one emission line:
  - `line`：Line name (e.g., "Lyα", "C IV", "O[II]")
