...
6. **[O II] unresolved doublet morphology** — [O II] 3727 is a close doublet (3726.0/3729.0 Å, rest sep 2.8 Å). It appears as a single blended peak in the line catalog, but the raw spectrum carries morphological signatures of the blend. **[O III]b 5008.2 is a TRUE single line.** When the SAME observed feature is claimed as [O II] by one hypothesis and [O III]b by another, use `grep_kb(pattern="O II.*doublet|unresolved|3726", C=5)` and `read_spectrum_region` ±25 Å. The rising edge forms a **morphological continuum** (strongest→weakest [O II] evidence): **valley** (flux reverses on rising edge) → strongly favors [O II]; **plateau/stall** (flux constant for 2–4 pixels on rising edge) → moderately favors [O II]; **slope-change** (steep→shallow→steep rise rate, no reversal or stall) → moderately favors [O II]; **clean single Gaussian** (smooth monotonic rise) → favors [O III]b. A single Gaussian cannot produce a flux reversal, plateau, or kink in the rise rate. **Also check the [O II] vs Balmer amplitude hierarchy**: in typical ELG spectra [O II] should be comparable to or brighter than Hβ. If the claimed [O II] amplitude is < 0.5× the Hβ (or Hγ/Hδ) amplitude at the same redshift, this strongly penalizes the [O II] identification — a true [O II] is not dwarfed by Balmer lines. This combined morphology+amplitude test is the most powerful physical discriminator for the common [O II]-vs-[O III]b degeneracy.
...
- "z=X identifies the feature as [O III]b, but spectrum read shows rising-edge valley/plateau/slope-change + broadened FWHM — morphology matches [O II] unresolved doublet, not a true single line"
