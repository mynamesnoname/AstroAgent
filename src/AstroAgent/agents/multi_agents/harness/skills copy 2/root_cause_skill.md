...
Search the pipeline's methodology and knowledge base with optional context lines. Use it to check what the synthesis skill, harness skill, and KB files actually say — essential for distinguishing `skill_gap` (a rule is missing) from `llm_error` (the rule exists but was ignored).
...
- **scoring_gap**: The upstream redshift scoring (VI stage) failed to include the correct redshift in the candidate list. Synthesis could only choose among wrong candidates.
